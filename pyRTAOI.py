...
class Worker(QObject):
    ...
    refreshTrajPlot_signal   = pyqtSignal(object,name = 'refreshTrajPlot')
    ...
    def __init__(self, **kwargs ):
        ...
        try:
            ...
            self.TrajBuffer =  np.zeros([1,self.BufferLength])
            ...
            #                   print(all_bs_level)
                        ...
                        if sens_stim_idx>num_bs_trials:
                            if framesProc == stim_frames[sens_stim_idx-1]+ offset_frames:
                                ...
                                if this_trialOrder ==2:# p['ROIsumAbove']  stim type 2 will be stimulated when traj>0
...
#        self.drawOn = False
        ...
        self.Rejectcontour_item = pg.ScatterPlotItem()
        self.Rejectcontour_item.setBrush(self.scatterbrush)
        self.graphicsView.addItem(self.Rejectcontour_item)       
...
#        self.createRejectMask_pushButton.clicked.connect(self.autoCreateRejectMask)
        ...
        self.CNNFilter_pushButton.clicked.connect(self.RunCNNFilter)
        self.getCNNPrediction_pushButton.clicked.connect(self.getCNNPrediction)
        ...
        self.hideROIIdx_pushButton.clicked.connect(self.deleteTextItems) 
        self.clearRejectContour_pushButton.clicked.connect(self.clearRejectContour)
    ...
    def loadResultPath(self):
        ...
        if result_path:
            ...
            for i in range(0,len(accepted_idx)):
                self.getROImask(coms[i,1], coms[i,0],glb_idx = accepted_idx[i])
            self.showROIIdx()
...
#       del self.c['cnm2']  commented out 20190711
    ...
    def RunCNNFilter(self, init=False):
        if p['CNN_predictions']==[]:
            print('Run CNN first!')
            return
        try:
            ...
            if self.c['CNN_filter']:
                ...
                idx_keep_CNN = np.where(p['CNN_predictions'] >= thresh_cnn)[0].tolist()
                idx_rem_CNN = np.where(p['CNN_predictions'] < thresh_cnn)[0].tolist()
                print(str(len(idx_keep_CNN)) + ' rois left post filtering')

                self.c['thresh_cnn'] = thresh_cnn
                print('idx(global) below thresh', idx_rem_CNN)
                
                # convert global index to ROIlist index
                accepted = self.c['cnm2'].accepted
                both = set(accepted).intersection(idx_rem_CNN)
...
#
    ...
    def getCNNPrediction(self,init):
        # get CNN prediction score; this if for all components, including rejected
        if init:
        ...
        predictions[np.isnan(predictions)] = 0  # exclude nan
              # save prediction somewhere  
        p['CNN_predictions'] = predictions[:, 1]
...
#       print(event.key())
    ...
    def getROImask(self, thisROIx, thisROIy,glb_idx = 0):
        if self.ALL_ROI_SELECTED == False:
            ...
            self.ROIlist[self.thisROIIdx]["glb_idx"] = glb_idx
            self.thisROIIdx += 1
...
#%% load/save configuration
    ...
    def saveResults(self):
        # imaging results should be saved after aquisition, this is for saving filter results
        try:
            save_dict = dict()
#            save_dict['cnm2'] = self.c['cnm2'] #self.cnm2
#            save_dict['com_count_init'] = self.c['K_init']
#            save_dict['coms'] = self.c['coms']
            save_dict['CNN_predictions'] = p['CNN_predictions']
            save_dict['CNN_thresh'] = self.CNNFilter_doubleSpinBox.value()
            save_dict['p'] = p         
            save_object(save_dict, p['saveResultPath'])
            print('results saved as'+p['saveResultPath'])
            pkl2mat(p['saveResultPath'], IF_CNMF = False)
    ...
    def browseResultPath(self):
        daytimestr = time.strftime("%Y%m%d-%H%M%S")
        timestr = daytimestr[-6:]
        saveResultsPath = QFileDialog.getSaveFileName (self, 'Select save path', "", "(*.pkl)")
        p['saveResultPath'] = saveResultsPath[0].replace(".pkl",'_FiltResult'+timestr+'.pkl')
        self.saveResultPath_lineEdit.setText(p['saveResultPath'])
