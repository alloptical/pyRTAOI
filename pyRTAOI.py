''''
 Dependences:
 follow instructions in these links to install all dependencies
            ...
            while self.FLAG_SCANNING:
                ...
                if not num_samples:
                    ...
                    if self.UseONACID: # HAVENT TESTED ONLINE YET
                        ...
                        # add data to buffer
                        try:
                            RoiBuffer[:com_count, BufferPointer] = cnm2.C_on[accepted,t] #[gnb:gnb+com_count, t]
                        ...
                        if framesRecvd>refresh-1: #frame_count>BufferLength-1:
                            if LastPlot == refresh:
                                if p['plotOn']:
                                    self.refreshPlot_signal.emit(RoiBuffer[:com_count,:])
                                if p['displayOn']:
...
#%% OFFLINE ANALYSIS
        if self.FLAG_OFFLINE and self.UseONACID:
            self.status_signal.emit('Loading video')

            # load movie
            print('Loading video')                
            Y_ = cm.load(p['moviePath'], subindices=slice(0,1000,None))  # T1
            print('Video loaded')
...
#                            try:
                            if opsin_mask.size:
                                ...
                                # error here now
# C:\ci\opencv_1512688052760\work\modules\core\src\arithm.cpp:225: error: (-209) The operation is neither 'array op array' 
# (where arrays have the same size and type), nor 'array op scalar', nor 'scalar op array' in function cv::binary_op
...
#                    if displayOn:
#                        self.thresh_signal.emit(ROIlist_threshold)
            ...
            try:
                ...
                if show_traces:
                    img = Cn_init
                    self.sendTraces_signal.emit(cnm2, t, img)
            except Exception as e:
                print(e)
            ...
            # this does not show init frames for epochs > 1 and may not show all frames of last epochs
...
#            YrA = noisyC[cnm2.gnb:cnm2.M] - C
            ...
            save_results = True
            if save_results:
                np.savez(p['moviePath'][:-4] + '_offline_DS_ ' + str(ds_factor) + '.npz',
                         Cn=Cn, Cf = cnm2.C_on, A=A, b=b, C=C, f=f, noisyC=noisyC, b_trace=b_trace,
                         OASISinstances = cnm2.OASISinstances, deconvolved = deconvolved, coms=coms,
                         coms_post = coms_post, dims=cnm2.dims, ds_factor=ds_factor, tottime=tottime,
                         shifts=shifts)
                
#                         Ab=cnm2.Ab, Cf=cnm2.C_on, b=cnm2.b, f=cnm2.f,
...
#        mousePressEvent
#        mouseReleaseEvent
        #leaveEvent(self, ev):  ## inform items that mouse is gone
        
        # this doesnt do anything
        from PyQt5 import QtCore
        #dir(PyQt5.QtCore.Qt) for keys
...
#        self.maskInitialise_pushButton.clicked.connect(self.maskInitialise)
        ...
        self.UseOpsinMask_radioButton.toggled.connect(self.switch_useOpsinMask) # addded
        self.UseOnacidMask_radioButton.toggled.connect(self.switch_useOnacidMask) # added
        ...
        # dir(pg.GraphicsScene) for all possible methods of scene
#        self.graphicsView.scene().dragEnterEvent.connect(self.exampleDrop)
        # error: 'builtin_function_or_method' object has no attribute 'connect'
...
#        print( self.ROIlist[ROIIdx]["ROIcolor"] )
    ...
    def resetAll(self):
        ...
        if(retval==QMessageBox.Ok):
            ...
            # clear table   
            self.updateTable()
            self.plotItem.clear() # added
            
            self.updateFrameInfo(0) # added
...
#            print('sending coords, bl connected = '+ str(self.bl.CONNECTED))
    ...
    def plotSTA(self):
        ...
        try:
            self.staTracesFig, self.axs = plt.subplots(int(plot_rows),int(plot_cols), 
                                   facecolor='w', edgecolor='k', frameon=False)
            
#            self.staTracesFig = fig
            self.staTracesCanvas = FigureCanvas(self.staTracesFig)
#            self.axs = self.staTracesFig.add_axes(axs)
    
            self.axs = self.axs.ravel()
            for i in range(num_rois):
                for j in range(num_trials):
                    try:
                        self.axs[i].plot(sta_traces[i,j,:],color = (.5,.5,.5))
                        self.axs[i].plot(sta_trial_avg[i,:], color=(0,0,0))
                        self.axs[i].set_title('roi'+str(i)+' amp = '+str("%.2f"%sta_tiral_avg_amp[i]))
                        self.axs[i].set_aspect('auto')
                        self.axs[i].set_frame_on(False)
                        self.axs[i].set_adjustable('box')
                    except Exception as e:
                        print(e)
    ...
    #        plt.show()
            ...
            print('adding figure to layout')
    #        self.staTracesFig = fig
    #        self.staTracesCanvas = FigureCanvas(self.staTracesFig)
            self.staPlot_gridLayout.addWidget(self.staTracesCanvas)
            self.updateStatusBar('STA traces plotted in plot tab')
        except Exception as e:
            print(e)


    def resetFigure(self):  # TODO: does not free memory. easy to go too high
        try:
#            self.fig.clear()
#            plt.close(self.fig)
##            plt.close('all')
...
#        if opsin_seeded and mask_seeded: # temp
#            self.updateStatusBar('Conflict! Select only one mask option!')
#            return
        cnmf_init = not (opsin_seeded or mask_seeded)
        print(opsin_seeded)
        print(cnmf_init)
        
#        if mask_seeded and (not self.A_loaded.size): # not needed if button disabled until mask loaded
#            self.UseOnacidMask_radioButton.setChecked(False)
#            mask_seeded = False
#            cnmf_init = True
#            self.updateStatusBar('No mask provided; using standard CNMF to initialise...')
            ...
            if self.A_opsin.size:
                try:
                    if movie_ext == '.tif':
                        ...
                        # Filter seeded - move inside initialise?
                        CNN_filter = True
                        if CNN_filter:
                            print('Filtering the components')
                            A = init_values['cnm_init'].A
                            gSig = init_values['cnm_init'].gSig
                            thresh_cnn = 0.1  # threshold for CNN classifier
                            
                            predictions, final_crops = evaluate_components_CNN(
                                A, init_values['cnm_init'].dims, gSig, model_name=os.path.join(caiman_datadir(), 'model', 'cnn_model'))
...
#                return
    ...
    def showCellsOnMask(self):
        if self.c != {}:
        ...
        elif self.A_loaded.size:  # temp
            dims = self.opsin_mask.shape
            print(dims)
            overlap = []
            opsin = []
            self.opsin_overlap_ratio = self.overlapRatio_doubleSpinBox.value()
            ...
            summed_A = np.hstack((self.A_opsin, onacid_mask))
            summed_mask = np.reshape(np.array(summed_A.sum(axis=1)), dims, order='F')
            pl.figure();pl.imshow(summed_mask)
            pl.colorbar()
