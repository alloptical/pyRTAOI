...
for v = 1:num_trial_types
    ...
    trial_indices.(['correct_stim_' num2str(this_stim) '_var_' num2str(this_var)  ]) = find(trials.correct==1&trials.stim_type==this_stim&trials.stim_var == this_var);
    trial_indices.(['incorrect_stim_' num2str(this_stim) '_var_' num2str(this_var)  ]) = find(trials.incorrect==1&trials.stim_type==this_stim&trials.stim_var == this_var);
    trial_indices.(['miss_stim_' num2str(this_stim) '_var_' num2str(this_var)  ]) = find(trials.miss==1&trials.stim_type==this_stim&trials.stim_var == this_var);
...
%% GET CELL IDENTITY
% stimulus AUC calculated from correct trials, highest contrast and amp
num_shuf = 300;
...
opt.target_idx_fd = 'stim';
opt.trigger_idx_fd = 'stim';
...
for i = 1:plot_num_cells
    ...
    plot(cell_struct(i).('correct_stim_1_var_3'),'color',trial_color.L1,'linewidth',1)
    plot(cell_struct(i).('correct_stim_2_var_6'),'color',trial_color.L2,'linewidth',1)
...
%% decoder - to debug below this point..
% add some fields to trialcolor
for i = 1:numel(trial_types)
       this_fd = trial_types{i};
       if contains(this_fd,'correct_stim_1')
           trial_color.(this_fd) = trial_color.L1;
       end
       if contains(this_fd,'incorrect_stim_1')
           trial_color.(this_fd) = trial_color.ipsi_L1;
       end
       
       if contains(this_fd,'correct_stim_2')
           trial_color.(this_fd) = trial_color.L2;
       end
       if contains(this_fd,'incorrect_stim_2')
...
fa_opt.bin_size = 1;
fa_opt.gocue_bin = floor(opt.gocue_frame/fa_opt.bin_size);
fa_opt.stim_bin = ceil(opt.stimon_frame/fa_opt.bin_size);
...
fa_opt.idx_fields = {'stim'};
fa_opt.fd_names = {'correct_stim_1_var_3','incorrect_stim_1_var_3','correct_stim_2_var_6','incorrect_stim_2_var_6'};
