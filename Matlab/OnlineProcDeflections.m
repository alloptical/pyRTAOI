...
for i = 1:num_cells
    this_cell_trace = cnm_struct(cell_struct(i).cnm_idx).deconvC_full;
    this_online_trace = cnm_struct(cell_struct(i).cnm_idx).onlineC_full;
...
stim_fds = {'stim_1_var_3_correct','stim_2_var_6_correct'}; 
...
disp('got stim auc')
%% get choice auc
choice_fds = {'stim_2_var_5_incorrect','stim_2_var_5_correct'}; 
...
%% get cells corelated with relative change 
...
%% discard cells with low snr
cell_snr = cell_bs./cell_sd.^2;
high_snr_cell_idx = find(cell_snr>.3);
for  c = 1:num_cells
...
coding_direct = cell2mat(arrayfun(@(x)mean(mean(raw_cell_struct(x).(choice_fds{1})(opt.sta_avg_frames,:),2)),cell_idx_struct.port,'UniformOutput', false))...
    - cell2mat(arrayfun(@(x)mean(mean(raw_cell_struct(x).(choice_fds{2})(opt.sta_avg_frames,:),2)),cell_idx_struct.port,'UniformOutput', false));
...
if opt.IF_USE_HMM
    ...
    plot_pop_vectors(hmm_struct,hmm_opt.plot_fds,num_states,hmm_opt,'IF_MEDIAN',0,'plot_num_cols',4);
...
cd_traj_struct = get_cd_projection(raw_cell_struct(cell_idx_struct.port),coding_direct);
...
stim_opt = fa_opt;
stim_opt.fd_names = {'stim_1_var_9_correct','stim_2_var_9_correct'}; % stim 1 will be positive
...
stim_opt.IF_FRAMEWISE =1;
stim_struct = {};
stim_proj_struct = {};
disp('Rnning stim decoder...')
tic
stim_struct =  get_binary_classifier( stim_struct,traj_struct, stim_opt,'IF_FRAMEWISE',stim_opt.IF_FRAMEWISE,'fd_names',stim_opt.fd_names);
...
choice_opt.frames_to_avg = [60:90]; %using baseline to decode choice
...
tic
choice_struct =  get_binary_classifier( choice_struct,traj_struct, choice_opt,'IF_FRAMEWISE',choice_opt.IF_FRAMEWISE,'fd_names',choice_opt.fd_names);
...
plot_pop_vectors(choice_proj_struct,test_fd_names,1,opt,...
    'ylimit',[-20 20],'plot_ylabel','Projection','plot_num_cols',6,'IF_PLOT_RAW_ONLY',1)
...
choice_after_stim_struct =  get_binary_classifier( choice_after_stim_struct,stim_proj_struct, choice_opt,...
    'IF_CROSSVAL',0,'IF_FRAMEWISE',0,'fd_names',decod_fds);
...
[choice_proj_struct] = get_projections(raw_cell_struct(cell_idx_struct.(opt.trigger_idx_fd)),norm_weights,test_fd_names,'bias',-norm_thresh,'IS_CELL_STRUCT',1);
...
plot_pop_vectors(choice_proj_struct,decod_fds,1,opt,...
        'ylimit',[-5 5],'plot_ylabel','Projection','IF_PLOT_RAW_ONLY',0)
...
plot_pop_vectors(choice_proj_struct,test_fd_names,1,opt,...
       'ylimit',[-1 1],'plot_ylabel','proj to choice-stim','plot_num_cols',3,'IF_PLOT_RAW_ONLY',1)
suptitle('Choice-stim decoder')
...
condition_stim_var  = [ 3  5];
condition_type = {[103],[105]};
