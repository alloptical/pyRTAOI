function [cell_struct] = get_cell_auc(cell_struct,stim_fds,fd_name,opt)
num_shuf = 300;
...
cell_struct(i).([fd_name '_zscore']) = (correct_stimulusAUC-mean(shuf_stim_auc))/std(shuf_stim_auc);
cell_struct(i).(fd_name) = correct_stimulusAUC;
end
