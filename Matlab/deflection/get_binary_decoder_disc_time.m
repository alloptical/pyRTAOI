function [ decod_struct ] =  get_binary_decoder_disc_time( proj_struct, decod_struct,fds,opt,varargin )
...
if isempty(threshold)
    if ~IF_USE_DB
        if IF_FRAMEWISE
            this_thresh = decod_struct.thresh_framewise;
        else
...
for f = 1:num_frames
    ...
    if IF_USE_CROSSVAL_RESULT
        this_shuf_accuracy = decod_struct.all_accur_shuf(:);
    else
...
decod_struct.framewise_hr = hit_rate;
decod_struct.framewise_fa = fa_rate;
decod_struct.disc_frames = disc_frames; % discrimination frames by hit rate and false alarm rate
decod_struct.disc_frame = disc_frame;
...
decod_struct.shuf_disc_frames = shuf_disc_frames; % discrimination frames by shuffling
decod_struct.shuf_disc_frame = shuf_disc_frame;
