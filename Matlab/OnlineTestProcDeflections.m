...
% cell_sd = cell2mat(arrayfun(@(x)std(reshape(cell_struct(x).sta_traces(easy_trial_idx,:),[1,numel(easy_trial_idx)*opt.trial_length])),1:num_cells,'UniformOutput', false));
% for i = 1:num_cells
%     % normalise to easy trials sd
%     cell_struct(i).sta_traces = cell_struct(i).sta_traces - mean(cell_struct(i).sta_traces(:,opt.sta_baseline_frames:opt.sta_stimon_frame),2)./cell_sd(i);
% 
%     cell_struct(i).sta_trace = mean(cell_struct(i).sta_traces ,1);
% end
...
%% GET CELL IDENTITY -  should load indices used in baseline session
% (if no cells were removed between baseline and condition session then should be fine)
...
%% load baseline decoder (file named as 'OutputParams')
...
test_opt.fd_names = {'stim_1_var_3_correct','stim_1_var_3_incorrect','stim_1_var_3_miss',...
                     ...
                     'stim_2_var_7_correct','stim_2_var_7_incorrect','stim_2_var_7_miss'...
};
proj_struct = struct();
...
plot_pop_vectors(proj_struct,test_opt.fd_names,1,test_opt,...
        'ylimit',[-20 40],'plot_ylabel','Projection','plot_num_cols',3,'IF_PLOT_RAW_ONLY',1)
