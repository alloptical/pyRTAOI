...
test_fd_names = {'stim_1_var_9_correct','stim_1_var_9_incorrect','stim_1_var_9_miss',...
                'stim_1_var_3_correct','stim_1_var_3_incorrect','stim_1_var_3_miss',...
                     ...
                     'stim_1_var_4_correct','stim_1_var_4_incorrect','stim_1_var_4_miss',...%                      'stim_3_var_2_correct','stim_3_var_2_incorrect','stim_3_var_2_miss',...
...
[all_stim_proj_struct] = get_projections(cell_struct(cell_idx_struct.(opt.trigger_idx_fd)),stim_norm_weights,test_fd_names,...
    'proj_struct',all_stim_proj_struct,'bias',-stim_norm_thresh,'IS_CELL_STRUCT',1);

plot_pop_vectors(all_stim_proj_struct,test_fd_names,1,opt,...
        'ylimit',[-5 5],'plot_ylabel','Projection','plot_num_cols',6,'IF_PLOT_RAW_ONLY',0)
%% check if the trials used for training look normal
plot_pop_vectors(all_stim_proj_struct,stim_opt.fd_names,1,opt,...
        'ylimit',[-5 5],'plot_ylabel','Projection','plot_num_cols',2,'IF_PLOT_RAW_ONLY',0)
...
for f = 1:numel(test_fd_names)
    ...
    if ~isempty(all_stim_proj_struct.(fd))
        this_raw = 1+double( mean(all_stim_proj_struct.(fd)(:,stim_opt.frames_to_avg),2)<0);
