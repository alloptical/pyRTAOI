...
def make_sta_file(file_full_name = '',save_full_name = '', stim_frames = [], 
	...
	print(len(frame_detected))


	# get data
	cnm = file_data['cnm2']
	init_com_count = file_data['init_com_count']
	num_init_frames = frame_detected[0] # has to initialise with something
...
def plotSTAtraces(sta_traces=[],frames_to_average = range(30,60), IF_PLOT = True):
	if sta_traces == []:
...
if __name__ == '__main__':
	...
	sta_traces = make_sta_file(stim_frames_field = 'stim_frames_caiman')
